...
A command-line solver for [LinkedIn Queens](QUEENS.md) puzzles implemented purely through heuristic propagation. The solver does **not** use backtracking. It either solves the board with the heuristics catalogued in [HEURISTICS.md](HEURISTICS.md), or it reports the partially-solved state and exits with a non-zero status.
