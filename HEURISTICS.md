...
For the data model, vocabulary (`regions`, `queens`, `dead`, `is_live`, live / dead / queen states), and rules of the game, see `QUEENS.md`. The pseudocode here uses those directly.
...
Strategy: apply the rules in two tiers. The cheap tier is every rule above `placement-forces-contradiction`: the baseline rules, the polyomino shape rules, and the pigeonhole N-regions rules. With the precomputed structures in place each cheap-tier rule costs `O(B)` per pass (except a dirty N-regions pass at `O(B²)`), so the driver can run them all each pass and converge to a cheap-tier fixed point quickly. Only when that fixed point is reached and the puzzle is still unsolved should the driver invoke `placement-forces-contradiction`. If the H2 pass marks any cell dead, loop back to the cheap tier and repeat. Stop when a full cycle of both tiers leaves everything unchanged. For LinkedIn boards (`B ≤ 11`) a full cheap-tier pass takes well under a millisecond, and an H2 pass is several times that but still bounded by `O(B²)` bit-ops. In practice the cheap tier alone solves essentially every shipped puzzle. The extended heuristics matter for harder hand-designed variants, for verifying uniqueness, and for the relatively small set of shipped puzzles that stall under the cheap tier.
